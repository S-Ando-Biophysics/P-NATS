...
cleanup_x3dna() {
  rm -f \
    *.out *.outs *.dat *.r3d *.scr \
    ...
    auxiliary.par bp_step.par cf_7methods.par bp_helical.par
}
...
convert_cif_to_pdb() {
  ...
  [[ -f "$input_cif" ]] || {
    echo "[ERROR] The CIF file was not found: $input_cif" >&2
    return 1
  }
...
PY
  ...
  [[ -f "$output_pdb" ]] || {
    echo "[ERROR] Failed to generate the PDB file: $output_pdb" >&2
    return 1
  }
}
...
normalize_initial_pdb() {
  ...
  [[ -f "$pdb" ]] || {
    echo "[ERROR] The PDB file was not found: $pdb" >&2
    return 1
  }
  ...
  ' "$pdb" "$pdb" > "$tmp" || {
    echo "[ERROR] Failed while normalizing the initial PDB file: $pdb" >&2
    return 1
  }
...
split_na_chains() {
  ...
  if [[ ${#chains[@]} -eq 0 ]]; then
    echo "[ERROR] No chain IDs could be extracted from the nucleic-acid PDB file: $na_pdb" >&2
    return 1
  fi
  ...
  if [[ ${#valid_chains[@]} -eq 0 ]]; then
    echo "[ERROR] No valid chain-specific PDB files were generated from: $na_pdb" >&2
    return 1
  fi
...
rewrite_chain_residues_from_par() {
  ...
  [[ -f "$chain_pdb" ]] || {
    echo "[ERROR] The chain PDB file was not found: $chain_pdb" >&2
    return 1
  }
  [[ -f "$par_file" ]] || {
    echo "[ERROR] The PAR file was not found: $par_file" >&2
    return 1
  }
  ...
  ' "$par_file" "$chain_pdb" > "$tmp" || {
    echo "[ERROR] Failed while rewriting residue names in: $chain_pdb" >&2
    return 1
  }
...
assemble_purified_pdb() {
  ...
  for f in "${chain_files[@]}"; do
    [[ -f "$f" ]] || {
      echo "[ERROR] Chain file not found during assembly: $f" >&2
      return 1
    }
    awk '/^(ATOM  |HETATM)/ { print }' "$f" >> "$output_pdb" || return 1
  done
...
process_chain_without_rebuild() {
  ...
  find_pair -s "$chain_pdb" | analyze >/dev/null 2>&1 || {
    echo "[ERROR] find_pair/analyze failed for: $chain_pdb" >&2
    cleanup_x3dna
    return 1
  }

  [[ -f bp_helical.par ]] || {
    echo "[ERROR] bp_helical.par was not generated for: $chain_pdb" >&2
    cleanup_x3dna
    return 1
  }

  mv bp_helical.par "$par_file" || {
    echo "[ERROR] Failed to rename bp_helical.par to: $par_file" >&2
    cleanup_x3dna
    return 1
  }
...
run_chain_workflow_from_pdb() {
  ...
  [[ -f "$pdb" ]] || {
    echo "[ERROR] The input PDB file was not found: $pdb" >&2
    return 1
  }

  get_part -n -d "$pdb" "$na_pdb" || {
    echo "[ERROR] get_part failed for: $pdb" >&2
    return 1
  }

  [[ -f "$na_pdb" ]] || {
    echo "[ERROR] get_part did not generate: $na_pdb" >&2
    return 1
  }
  ...
  if [[ ${#chains[@]} -eq 0 ]]; then
    echo "[ERROR] No chains were detected in: $na_pdb" >&2
    return 1
  fi
  ...
  for ch in "${chains[@]}"; do
    ...
    process_chain_without_rebuild "$chain_pdb" || return 1
    processed_chain_files+=("$chain_pdb")
  done

  assemble_purified_pdb "$purified_pdb" "${processed_chain_files[@]}" || return 1

  cp "$purified_pdb" "${outdir}/" || {
    echo "[ERROR] Failed to copy output to: ${outdir}/${purified_pdb}" >&2
    return 1
  }
...
process_downloaded_assembly() {
  ...
  [[ -f "$cif" ]] || {
    echo "[ERROR] The CIF file was not found: $cif" >&2
    return 1
  }
...
process_local_pdb() {
  ...
  cp "$input_abs" "./${prefix}.pdb" || return 1
  normalize_initial_pdb "${prefix}.pdb" || return 1
  run_chain_workflow_from_pdb "${prefix}.pdb" "$prefix" "$outdir" || return 1
}
...
process_local_cif() {
  ...
  cp "$input_abs" "./${prefix}.cif" || return 1
  convert_cif_to_pdb "${prefix}.cif" "${prefix}.pdb" || return 1
  normalize_initial_pdb "${prefix}.pdb" || return 1
  run_chain_workflow_from_pdb "${prefix}.pdb" "$prefix" "$outdir" || return 1
}
...
main() {
  ...
  if [[ "$MODE" == "pdbid" ]]; then
    ...
    while true; do
      ...
      if (
        ...
        success_assemblies+=("$asm")
        log "Assembly ${asm} processed successfully."
      else
        failed_assemblies+=("$asm")
        warn "Assembly ${asm} failed, but skipping to next."
      fi
    ...
    if [[ ${#failed_assemblies[@]} -gt 0 ]]; then
      warn "Failed assemblies: ${failed_assemblies[*]}"
    fi

  else
    case "${INPUT_FILE##*.}" in
      pdb|PDB)
        process_local_pdb "$start_dir/$INPUT_FILE" "$start_dir" || die "Local PDB processing failed."
        ;;
      cif|CIF)
        process_local_cif "$start_dir/$INPUT_FILE" "$start_dir" || die "Local CIF processing failed."
        ;;
      *)
  ...
  cd "$start_dir"
  log "All processing has been completed."
}
